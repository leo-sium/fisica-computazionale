...
#define N 2 // <- dimensioni del sistema di Eq. Diff.
#define T_start  0.  // <- inizio di evoluzione del sistema di Eq. Diff.
#define T_end   250.   // <- fine di evoluzione del sistema di Eq. Diff.
...
int main() {
  ...
  y[0]=0.; // <- condizioni iniziali del sistema di Eq. Diff.
  y[1]=1.; // <- condizioni iniziali del sistema di Eq. Diff.
  T_steps=1000000;  // <- numero di passi di integrazione

  char c[4] = "RK2" ;
  // integratore(f,T_steps,"Eulero");
   integratore(f,T_steps,c);
...
double f1(double t, double *v)
{
  double gamma = 0.1;
  double A = 0.1;
  return -sin(v[0]) -gamma*v[1] + A*sin(t*2./3.);
}
